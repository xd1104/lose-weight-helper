...
| `09-floor.js` | 熱量上限語意（減脂顯示「上限」）、目標低於 BMR 的警告與消失條件、**三段語意**（綠／黃＝超過上限但未超 TDEE／紅＝超過 TDEE；v6.4 起畫在進度條上，不是圓環） |
...
| `26-push.js` | **每日提醒**：索引摘要、iPhone 分頁要先加入主畫面、打開→要權限→訂閱→寫進 push.md（時間/時區/使用者）、先選時間再打開、被拒的說明、關閉要真的退訂、⚠️改自己的設定不可以蓋掉別台裝置與女友那筆 |
| `27-fav.js` | **常吃清單的分組與折疊**：分組是從「都在哪一餐吃」算出來的（不用手動標）、預設只展開現在要記的那一餐、換餐別跟著換、預設只列吃過 2 次以上、搜尋一律搜全部、短清單不分組也不過濾 |
| `28-copy.js` | **從另一位使用者複製一餐**：對方沒記那一餐就完全不顯示、預設全選可挑掉、整批調倍數、複製過去的是自己的數字並進自己的常吃、已記過的不再出現（不重複加）、⚠️只有拉沒有推（不可以動到對方的紀錄） |
| `29-friction.js` | **「懶得記」的兩個對策**：校準摘要上首頁（只在有話說時出現、講得出至少漏記多少、點進去看完整版）、AI 回 6 項以上預設先合併成一筆（「分開列出」回得去、資料沒有不見） |
...
`26-push.js` 把 `Notification` 與 `pushManager` 換成假的——無頭 Chromium 沒有推播服務，
真的訂閱不起來。所以它驗的是「app 拿到訂閱之後做了什麼」，不是推播本身。
**真的會不會跳只能在加入主畫面的 iPhone 上驗**。
送出端（VAPID 簽章、誤點、時區、去重）另外由 `test/reminders.js` 涵蓋，
那支會自己起一台假的推播服務，跑得到真的 HTTP 往返。
...
| `30-home.js` | **首頁改版（v6.4）**：最上面是體重趨勢（講得出 kg／週 與一個月會怎樣、在瘦綠在胖黃、點進歷史不會卡在載入中）、熱量降級成一條但三段語意不變、三大營養素搬去營養頁但明細留一個寫得出蛋白質缺口的入口、四餐＋運動一列一餐且一律預設收起來、體重不夠 3 筆就不畫趨勢、⚠️舊功能（講評／備註／日期切換／FAB／四個分頁）逐項還在 |
