...
| `24-ui.js` | 第二輪使用者視角稽核：常吃熱量帶單位、手動記錄有份量欄、編輯不再有孤兒的時間欄、中文配全形括號、短清單不放搜尋框 |
| `25-async.js` | 非同步流程的中斷：AI 失敗要回得到輸入頁（字還在）、等待中關掉之後 sheet 不可以自己彈回來／不可以蓋掉別張、講評同理但結果照樣存下來 |
...
## 非同步路徑要自己裝 unhandledrejection 監聽

**unhandled promise rejection 不會變成 `pageerror`**，Playwright 預設抓不到。
`.then`／`.catch` 裡面炸掉的錯誤因此完全無聲——v5.0 那個「AI 失敗卡在轉圈圈」
就是這樣躲過好幾版的。驗非同步流程的測試要照 `25-async.js` 開頭那樣，
用 `addInitScript` 裝 `window.addEventListener('unhandledrejection', …)` 再斷言它是空的。

另外兩條同樣是踩過才知道的：

- **測「畫面卡住／被蓋住」的段落要包 try/catch**（`25-async.js` 的 `step()`）。
  壞掉時 `click` 會 timeout 直接炸掉整支，後面幾段就都跑不到，只看得到第一個錯誤。
- **不要用「第一列」定位**。列表照時間排，測試中途加的那筆是「現在」，
  上午跑跟下午跑排序不一樣 → 同一份程式碼會上午紅、下午綠。用名稱定位。
