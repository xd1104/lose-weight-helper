...
(async () => {
  ...
  /* 使用者對「50 公克」沒有概念，沒辦法判斷 AI 估得對不對；
     但他看得出「這顆有沒有一顆蛋那麼大」。所以份量要用看得到的東西講。 */
  check('份量要用看得到的東西描述，公克放括號', /看得到的東西/.test(sys));
  check('明講「約 50g」「一份」是壞的寫法', /壞的寫法.*無法驗證/s.test(sys));
  check('給了可以用的參照物', /一個拳頭≈一碗≈180g/.test(sys));
