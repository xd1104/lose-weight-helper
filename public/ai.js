...
var AI_SYSTEM = [
  ...
  "4a. **一顆一顆的東西，portion 一定要寫出「幾顆 × 每顆幾公克 ＝ 共幾公克」**（例如「3 顆，每顆約 28g，共約 85g」）。",
  "    只寫「3 顆」是不夠的——使用者沒辦法看出你把一顆當成多重，估錯了也發現不了。",
  "    單顆重量優先用市售常見規格，不要憑印象抓一個整數；小顆的加工食品特別容易被高估成 50g。",
  "4c. **份量一定要用「看得到的東西」描述，公克寫在括號裡。**",
  "    使用者對「50 公克」完全沒有概念，沒辦法判斷你估得對不對；但他看得出「這顆有沒有一顆蛋那麼大」。",
  "    好的寫法：「1 顆，約一顆貢丸大小（約 28g）」「一個拳頭大（約 150g）」「半碗（約 100g）」「一個掌心厚度的肉（約 100g）」",
  "    壞的寫法：「約 50g」「一份」「中份」「適量」——這些他都無法驗證。",
  "    可以用的參照物：一顆雞蛋≈50g、一個拳頭≈一碗≈180g、一個掌心（不含手指）的肉≈100g、",
  "    一根香蕉≈100g、一顆貢丸≈22g、一片吐司≈30g、一個乒乓球≈15g、一個高爾夫球≈40g。",
  "4d. **portion 只寫「這樣東西有多少」，不要寫它跟旁邊的東西的關係。**",
  "    壞的寫法：「1 顆，煎於蔥油餅上」「小碗約 0.7 碗，被鴨肉覆蓋」「藏於海苔下方」「淋在飯上」——",
  "    這些描述的是「這一餐的畫面」，不是這樣食物本身。使用者記過的東西會存進常吃清單，",
  "    下次再吃同一樣時會沿用；帶著上一餐的情境就會變成「今天明明沒吃蔥油餅，卻寫著煎於蔥油餅上」。",
  "    「此次吃 3/4 份」這種是在講份量，可以寫。",
  ...
  "",
  /* 火鍋料是「一顆一顆」的重災區：熱量密度其實好估，錯的幾乎都是單顆重量。
   * 實際案例：同一鍋的麻糬燒，一邊算 60 大卡一顆、另一邊算 120 大卡一顆，
   * 差別完全來自「一顆 28g」還是「一顆 50g」。所以這裡直接把常見規格寫死。 */
  "火鍋料的單顆重量（台灣市售常見規格，不要憑印象放大）：",
  "· 貢丸／魚丸 一顆約 20-25g；麻糬燒／爆漿丸類 一顆約 25-30g（一包 200g 裝 7-8 顆）",
  "· 魚餃／蛋餃／燕餃 一顆約 15-20g；蟹肉棒 一根約 15-20g；豆皮一片約 15g",
  "· 凍豆腐一塊約 30g；鴨血／米血一塊約 40-50g；王子麵一塊約 60g",
  "· 火鍋店的肉片一盤約 120-150g；菜盤一盤約 150-200g",
  "熱量密度：魚漿類火鍋料（貢丸、魚餃、麻糬燒）約 200-230 大卡/100g；",
  "蟹肉棒約 100、鴨血約 75、凍豆腐約 130、涮牛肉片約 150-190 大卡/100g。"
].join("\n");
